...
│    ├─ occurrences     → candidate navigation (T20 semantics)      │
│    ├─ annotations     → human notes, export opt-in only (T23)     │
│    ├─ findings/explanations → findings, coverage, plain language  │
│    └─ reports         → portable JSON / script-free HTML export, │
│                        selectable scope + preview (T23)              │
...
7. **Export.** Reports are portable JSON or script-free HTML with all
   evidence images inlined as strict data URLs, no external fetches, and no
   local filesystem paths. Export scope (full run or selection), opt-in
   human notes and page renders, and a pre-export preview are enforced at
   this boundary, not by convention. Identity is deterministic: the same inputs seal to

# Mac coordinator and Homebase worker

The owner ended Cloud execution for quota consumption and selected Devin Local
with SWE-2 Max on the Mac plus native ZCode on Homebase. Antigravity is inactive.
...
security boundaries and independent acceptance. ZCode owns defined components,
...
4. Revalidate stale acceptance inputs on the integrated candidate before G1.
   ...
   version comparison stay with SWE; examples, repeated experimental runs,
...
SWE has at most two active workers/reviewers, counting the lead when it implements
or reviews. ZCode has one. Global capacity is three. Writers yield for review;
an idle coordinator does not occupy a product slot. Run one heavy OCR/corpus/
performance job at a time, recording its holder in the task's Beads note. A SWE
review that executes tests on Homebase uses a separate review checkout and shares
that heavy-job limit with ZCode. Assign distinct dev-server ports to simultaneous
checkouts; the static app ports are defaults, not permission to reuse a busy port.

There is no deadline or obligation to finish the whole product in one run.
Continue useful ready work, fix ordinary failures, publish recoverable progress
and resume native sessions without new per-ticket owner prompts. Keep expensive
...
| --- | --- | --- |
| Devin | `/Users/zubair/Code/Projects/pdf project/original` on `main` | Native Local general subagents in isolated Mac task/review worktrees |
| ZCode | `/home/wertyp/.local/share/homebase-factory/projects/inkflip` on `main` | Native Agent/Goal, one isolated Homebase task worktree at a time |
...
Only the Mac integration session writes Beads. Mac linked worktrees read its
...
1. Mac: read the current pass, grants and blockers. Resume an unfinished grant on
   ...
   `python3 scripts/native_pass.py dispatch Txx --app devin|zcode`.
...
5. Mac: poll `python3 scripts/homebase_relay.py collect` during native coordination.
   ...
   merge work. The lead orders independent review, resolves findings, integrates
   and records actual acceptance before dispatching dependent work.

SWE reviews ZCode work independently; SWE implementation gets a separate Local
general reviewer. Use real session identities in evidence. Homebase reviewers
...
The coordinator polls with native waits around 30–60 seconds when nothing has
changed; it does not need a daemon, cron task, or mirrored queue. Continue other

...
| Distribution gate | `python3 scripts/check_distribution.py --release` + `python3 -m unittest discover -s tests/release -v` | Declared shipped files match actual bytes; license evidence and NOTICE consistent ([distribution-preparation.md](distribution/README.md)) |
...
Work is recorded, not asserted. Acceptance receipts bind an evaluated
commit to executed commands and real test counts; working records stay
outside source commits and are archived in local Git object storage. The
mechanics — receipt format, freshness rules, what counts as manual evidence
— are specified in [docs/ACCEPTANCE.md](ACCEPTANCE.md). Two habits matter
for any contributor:
...
The distribution surface is declared in `config/distribution-manifest.json`
and verified with `python3 scripts/check_distribution.py --release`
(exit 0 pass, 1 verification failures, 2 config error); its tests live in
`tests/release/distribution/`. The inventory/SBOM generator under
`scripts/distribution/` is deterministic — outputs depend only on input
content digests, never on HEAD or wall-clock — and writes to the local
ignored working tree by default. See
[distribution/README.md](distribution/README.md) for the full workflow.
